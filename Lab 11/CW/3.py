...
class SavingsAccount(Account):
    def __init__(self, type, account_number, balance, interest_rate, limit):
    ...
    def print_details(self):
        super().print_details()
        print(f"Interest Rate: {self.interest_rate}, Minimum Limit: ${self.limit}")
...
class FixedDepositAccount(Account):
    def __init__(self, type, account_number, balance, yr):
        super().__init__(account_number, balance)
        self.account_type = type
        self.yr = yr
    ...
    def withdraw(self, amount):
        if self.maturity >= self.yr:
            print(f"Withdrew ${amount}.00. New Balance: ${self.balance-amount}")
        else:
